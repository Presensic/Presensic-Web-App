...
async function run() {
  const { data, error } = await supabase.from('employees').select('id, name, face_setup_completed').eq('name', 'Sarvesh');
  console.log("Sarvesh Data:", JSON.stringify(data, null, 2), "Error:", error);
}
run();
