...
const mapLogFromDB = (dbLog: any, employeesList: any[]) => {
  ...
  return {
    ...
    inside_geofence: dbLog.inside_geofence,
    location_name: dbLog.location_name,
    location_address: dbLog.location_address,
    distance_from_office_meters: dbLog.distance_from_office_meters,
    ip_address: dbLog.ip_address,
    device_info: dbLog.device_info,
                      ...
                      <div className={`p-3 rounded-xl border flex items-center justify-between ${
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold font-mono">
                            {isInside ? `Verified Inside ${currentCompany?.geofence_radius || 150}m Geofence Radius` : `Outside Permitted Work Area (${distanceMeters != null ? `${distanceMeters}m away` : "Unknown distance"})`}
                          </span>
                        </div>
                        <span className="text-[9px] font-mono px-2 py-0.5 bg-white border rounded-full border-slate-100 shadow-3xs font-extrabold text-slate-600">
                          {distanceMeters != null ? `${distanceMeters}m offset` : "Unknown distance"}
                        </span>
                      </div>
                    </div>
